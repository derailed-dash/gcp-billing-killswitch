...
class TestDisableBilling(unittest.TestCase):
    ...
    @patch('main.BudgetServiceClient')
    @patch('main.logging.Client')
    def test_billing_disabled_for_multiple_projects(self, mock_logging, mock_budget_client, mock_billing_client):
        ...
        mock_budget = MagicMock()
        mock_budget.budget_filter.projects = ['projects/test-project-123', 'projects/test-project-456']
        mock_budget_client.return_value.get_budget.return_value = mock_budget
        ...
        self.assertEqual(mock_billing_client.return_value.update_project_billing_info.call_count, 2)
        mock_billing_client.return_value.update_project_billing_info.assert_any_call({
            'name': 'projects/test-project-123/billingInfo',
            'billing_account_name': ''
        })
        mock_billing_client.return_value.update_project_billing_info.assert_any_call({
            'name': 'projects/test-project-456/billingInfo',
            'billing_account_name': ''
        })
